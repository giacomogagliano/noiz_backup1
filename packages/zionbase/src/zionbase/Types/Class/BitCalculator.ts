...
export const calculateBitsForNumber: IcalculateBitsForNumber =
  function (x: number): number {
    return Math.ceil(Math.log2(x + 1));
  };

abstract class CalculationStrategy extends Strategy {
  abstract method(x: unknown): number;
  abstract name: string;
}

class NumberCalculationStrategy
...
export const calculateBitsForString: IcalculateBitsForString =
  function (x: string): number {
    return x.length * 8;
  };

class StringCalculationStrategy
  implements CalculationStrategy
{
...
export abstract class BitsCalculator {
  static NumberCalculationStrategy =
    NumberCalculationStrategy;
  static StringCalculationStrategy =
    StringCalculationStrategy;
  private strategy: CalculationStrategy;
  constructor(strategy?: CalculationStrategy) {
    if (!strategy)
      this.strategy =
        new BitsCalculator.NumberCalculationStrategy();
