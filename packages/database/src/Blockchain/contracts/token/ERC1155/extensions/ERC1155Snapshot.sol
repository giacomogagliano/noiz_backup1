...
abstract contract ERC1155Snapshot is ERC1155Struct {
    ...
    /// mapping accounts of users to token ID, and tokenID are mapped to snapshots
    mapping(address => mapping(uint256 => Snapshots))
        internal _accountBalanceOfIdsSnapshots;
    /// mapping tokend Ids to supply snapshots
    mapping(uint256 => Snapshots) private _totalSupplyOfIdSnapshots;
    ...
    /**
     * @dev Retrieves the total supply for each token ID.
     */
        ...
        returns (uint256)
    {
        ...
        return snapshotted ? value : totalSupply(id);
    }
    ...
    /**
     * can happen when a user calls: _transferBatch, _mintBatch, _burnBatch
     * balance of requires an array of addresses, while all the other functions
     * require one sender and one receiver. Therefor we pass each element in the
     * ids list into a for loop to provide a list of balances
     */
        ...
        private
    {
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            _updateSnapshot(
                _accountBalanceOfIdsSnapshots[account][tokenId],
    ...
    function _updateTotalSupplySnapshot(uint256 id) private {
        _updateSnapshot(_totalSupplyOfIdSnapshots[id], totalSupply(id));
    }
