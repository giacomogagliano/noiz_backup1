...
import type { erc1155IndividualUriSol } from "../src/types/contracts/contracts/zion/test";
type ERC1155IndividualURI =
  erc1155IndividualUriSol.ERC1155IndividualURI;

const CONTRACTNAME = "ERC1155IndividualURI";
const FILEPATH =
  "./contract-deployment-ERC1155IndividualURI.json";

const DEPLOYMESS = "Deploying contracts with the account:";
const BALANCEMESS = "Account balance:";
const ADDMESS = "Token address: ";
const SUCCESSMESS =
  "Contract deployed succesfully. Log reported in file: ";

async function main() {
  const [deployer] = await ethers.getSigners();
  const balanceInEth = await deployer.getBalance();
  const balance = ethers.utils
    .formatEther(balanceInEth)
    .toString();
