...
export function getFileFromPath_v1(
  ...
  if (isTs)
    return paths
      ...
      .map((arr) => arr[0])
      .map((path) => {
        if (path && path !== true) return [path, readFileSync(path).toString()];
      })
      .map((string) => {
        const regex = /(\/\/\**.*\*\/)|(?<=\s)\/\**.*\*\//g;
        let name: string;
        if (isApps)
          ...
          // @ts-expect-error
          name = string[0]
            .replace("/Users/WAW/Documents/Projects/ZION/apps/", "")
            .replace("/tsconfig.json", "");
        if (!isApps) {
          // TODO trovare solution typescript
          // @ts-expect-error
          name = string[0]
            .replace("/Users/WAW/Documents/Projects/ZION/packages/", "")
            .replace("@zionstate/", "")
        ...
        return [
          name,
          JSON.parse(string[1].replace(regex, "").replace("\n", "")),
        ];
      });
  if (!isTs)
    return paths
      ...
      .map((arr) => arr[0])
      .map((path) => {
        if (path && path !== true) return readFileSync(path).toString();
      })
      .map((string) => {
        // const regex = /(\/\/\**.*\*\/)|(?<=\s)\/\**.*\*\//g;
