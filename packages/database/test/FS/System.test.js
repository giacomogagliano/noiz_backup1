  ...
  describe(`STATIC METHODS`, () => {
    describe("System static method: buildTree()", () => {
      ...
      it("Should return root node with its 2 children", () => {
        ...
        expect(
          childrenPath.includes(`${initialPath}/is`)
        ).to.be.equal(true);
        expect(
          childrenPath.includes(
            `${initialPath}/fileInRoot.ts`
          )
        ).to.be.equal(true);
      });
      ...
      describe(`CLASS: TREE`, () => {
        ...
        // TODO #195 @giacomogagliano testare findByLevel()
        describe.skip(`Method findByLevel()`, () => {
          it(`dovrebbe tornare piu nodi??`, () => {
            const folderContenutoNelLivello1 = "Tree";
            expect(
              tree.findByLevel(1)[0].name
            ).to.be.equal(folderContenutoNelLivello1);
          });
        });
      });
