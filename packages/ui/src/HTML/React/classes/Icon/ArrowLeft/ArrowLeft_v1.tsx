import { IconPath, Icons } from "../../IconPath";

export type ArrowLeft_v1Data = "ArrowLeft_v1Data";
export type ArrowLeft_v1Booleans = "ArrowLeft_v1Booleans";
export type ArrowLeft_v1Props = "ArrowLeft_v1Props";
export type ArrowLeft_v1ClassProps =
  "ArrowLeft_v1ClassProps";
export type ArrowLeft_v1AsChild = "ArrowLeft_v1AsChild";

export const ArrowLeft_v2 = (
  <>
    <path d="M11.7071 5.29289C12.0976 5.68342 12.0976 6.31658 11.7071 6.70711L7.41421 11H19C19.5523 11 20 11.4477 20 12C20 12.5523 19.5523 13 19 13H7.41421L11.7071 17.2929C12.0976 17.6834 12.0976 18.3166 11.7071 18.7071C11.3166 19.0976 10.6834 19.0976 10.2929 18.7071L4.29289 12.7071C4.10536 12.5196 4 12.2652 4 12C4 11.7348 4.10536 11.4804 4.29289 11.2929L10.2929 5.29289C10.6834 4.90237 11.3166 4.90237 11.7071 5.29289Z"></path>
  </>
);

const arrowLeft_ = new IconPath();
arrowLeft_.name = Icons.arrowLeft;
arrowLeft_.JsxPath = ArrowLeft_v2;
arrowLeft_.filled = true;

export const arrowleft = arrowLeft_;
export const Arrowleft = arrowLeft_;
