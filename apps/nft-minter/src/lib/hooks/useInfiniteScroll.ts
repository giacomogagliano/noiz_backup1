...
export const useInfiniteScroll = <T>(posts: T[]): UseInfiniteScroll<T> => {
  const [isLoading, setIsLoading] = useState(false);
  const [page, setPage] = useState(1);
  const [hasDynamicPosts, setHasDynamicPosts] = useState(false);
  const [dynamicPosts, setDynamicPosts] = useState<T[]>(posts);
        ...
        loadMoreTimeoutRef.current = setTimeout(() => {
          fetch(`/api/posts/${page}`).then(async (resp) => {
            ...
            if (newPosts?.length) {
              ...
              setIsLastPage(newDynamicPosts?.length === json?.data["total"]);
